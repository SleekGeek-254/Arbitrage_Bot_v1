...
async function testcalculatePrice(_pairContract, pairz) {
    const [pairName] = _pairContract;
    const [x, y] = await eval(pairName).getReserves(pairz)
    return Big(x).div(Big(y))
}
...
async function simulate(amount, _routerPath1, _routerPath2, _token0, _token1) {
    const trade1 = await _routerPath1.getAmountsOut(amount, [_token0.address, _token1.address])
    const trade2 = await _routerPath2.getAmountsOut(trade1[1], [_token1.address, _token0.address])
