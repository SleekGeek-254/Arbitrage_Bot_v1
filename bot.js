...
const main = async () => {
  ...
  sPair.on('Swap', async () => {
    if (!isExecuting) {
      ...
      const wholeNumchecker = await checkNumber(priceDifference)
      const routerPath = await determineDirection(priceDifference)
...
const checkNumber  = async (priceDifference) => {
...
const checkPrice = async (exchange, token0, token1) => {
  ...
  const wholeNumchecker = await checkNumber(priceDifference)
  console.log(`Percentage Difference: ${wholeNumchecker}%\n`)

  return wholeNumchecker
}
